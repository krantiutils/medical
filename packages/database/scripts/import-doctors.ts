...
async function importDoctors(): Promise<void> {
  const csvPath = resolve(__dirname, "../../../data/nmc_doctors.csv");
  ...
  for await (const record of parser) {
    records.push(record as DoctorRecord);
  }
      ...
      try {
        ...
        if (!nmcNo || !rawName) {
          ...
          errorDetails.push({
            ...
            error: "Missing required field (nmc_no or full_name)",
          });
          return;
        }
        ...
        const result = await prisma.professional.upsert({
          ...
          create: {
            ...
            degree: record.degree?.trim() || null,
