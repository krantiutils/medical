...
test.describe("Pharmacy Products - Search and Filter", () => {
  ...
  test("should filter by search query", async ({ clinicOwnerPage }) => {
    ...
    const hasResults = await clinicOwnerPage.locator("table").isVisible({ timeout: 5000 }).catch(() => false);
    const noResults = await clinicOwnerPage.getByText(/no matching products|no products found/i).isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe("Pharmacy Products - Deactivate and Activate", () => {
  ...
  test("should toggle product active status via status filter", async ({ clinicOwnerPage }) => {
    ...
    const hasInactiveResults = await clinicOwnerPage.locator("table").isVisible({ timeout: 5000 }).catch(() => false);
    const noInactiveResults = await clinicOwnerPage.getByText(/no matching products|no products found/i).isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe("Pharmacy Products - Pagination", () => {
  ...
  test("should display pagination controls when products exceed page limit", async ({ clinicOwnerPage }) => {
    ...
    const hasPagination = await clinicOwnerPage.getByRole("button", { name: /previous/i }).isVisible({ timeout: 5000 }).catch(() => false);
