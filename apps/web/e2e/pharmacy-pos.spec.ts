...
test.describe("Pharmacy POS - Product Search", () => {
  ...
  test("should search products by name", async ({ clinicOwnerPage }) => {
    ...
    const hasResults = await clinicOwnerPage.getByText(/paracetamol/i).isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test("should search products by barcode", async ({ clinicOwnerPage }) => {
    ...
    const cartUpdated = await clinicOwnerPage.getByText(/cart \(1\)/i).isVisible({ timeout: 5000 }).catch(() => false);
    const errorMessage = await clinicOwnerPage.getByText(/product not found/i).isVisible({ timeout: 3000 }).catch(() => false);
    const stillEmpty = await clinicOwnerPage.getByText(/cart \(0\)/i).isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test("should display product info in search results", async ({ clinicOwnerPage }) => {
    ...
    const hasPrice = await clinicOwnerPage.getByText(/Rs\./).first().isVisible({ timeout: 5000 }).catch(() => false);
    const hasStock = await clinicOwnerPage.getByText(/in stock/i).first().isVisible({ timeout: 3000 }).catch(() => false);
    const noProducts = await clinicOwnerPage.getByText(/no products found/i).isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test("should show 'Add to Cart' option in search results", async ({ clinicOwnerPage }) => {
    ...
    const hasAddToCart = await clinicOwnerPage.getByText(/add to cart/i).first().isVisible({ timeout: 5000 }).catch(() => false);
    const noProducts = await clinicOwnerPage.getByText(/no products found/i).isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe("Pharmacy POS - Error Handling", () => {
  ...
  test("should handle barcode scan for non-existent product", async ({ clinicOwnerPage }) => {
    ...
    const hasError = await clinicOwnerPage.getByText(/not found/i).isVisible({ timeout: 5000 }).catch(() => false);
    const cartEmpty = await clinicOwnerPage.getByText(/cart \(0\)/i).isVisible({ timeout: 3000 }).catch(() => false);
