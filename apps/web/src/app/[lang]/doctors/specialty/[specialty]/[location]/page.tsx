...
async function getDoctorsBySpecialtyAndLocation(
  ...
  const where = {
    ...
    address: { contains: locationInfo.nameEn, mode: "insensitive" as const },
  };
...
export async function generateStaticParams() {
  const params: { lang: string; specialty: string; location: string }[] = [];
  for (const lang of locales) {
    for (const spec of SPECIALTIES) {
      for (const loc of LOCATIONS) {
        params.push({ lang, specialty: spec.slug, location: loc.slug });
      }
    }
  }
  return params;
...
export async function generateMetadata({
  params,
}: SpecialtyLocationPageProps): Promise<Metadata> {
  const { lang, specialty: specialtySlug, location: locationSlug } = await params;
  ...
  const description = lang === "ne"
    ...
    : `Find ${specialtyInfo.nameEn.toLowerCase()} doctors in ${locationInfo.nameEn}, Nepal. Browse verified NMC-registered specialists.`;

  const canonicalUrl = `${SITE_URL}/${lang}/doctors/specialty/${specialtySlug}/${locationSlug}`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: `${SITE_URL}/en/doctors/specialty/${specialtySlug}/${locationSlug}`,
        ne: `${SITE_URL}/ne/doctors/specialty/${specialtySlug}/${locationSlug}`,
      },
    },
  };
}
...
function generateJsonLd(
  ...
  return {
    ...
    url: `${SITE_URL}/${lang}/doctors/specialty/${specialtyInfo.slug}/${locationInfo.slug}`,
    medicalSpecialty: specialtyInfo.nameEn,
    areaServed: {
      "@type": "City",
      name: locationInfo.nameEn,
      addressCountry: "NP",
    },
    isPartOf: {
...
function buildPageUrl(
  ...
  const base = `/${lang}/doctors/specialty/${specialtySlug}/${locationSlug}`;
  if (page <= 1) {
    return base;
  }
...
export default async function SpecialtyLocationPage({
  ...
  const { professionals, totalCount, totalPages } = await getDoctorsBySpecialtyAndLocation(
    specialtyInfo,
    locationInfo,
    currentPage
  );

  const jsonLd = generateJsonLd(specialtyInfo, locationInfo, lang);
  ...
  const t = {
    title: lang === "ne"
      ...
      : `${specialtyInfo.nameEn} in ${locationInfo.nameEn}`,
    subtitle: lang === "ne"
      ? `${locName}मा ${specName} हेर्नुहोस्`
      : `Browse ${specialtyInfo.nameEn.toLowerCase()} doctors in ${locationInfo.nameEn}`,
    ...
    specialty: specName,
    otherLocations: lang === "ne" ? "अन्य स्थानहरू" : "Other Locations",
    noResults: lang === "ne" ? "कुनै डाक्टर भेटिएन" : "No doctors found",
    noResultsDesc: lang === "ne"
      ? `${locName}मा यस विशेषतामा हाल कुनै दर्ता भएको डाक्टर छैन।`
      : `There are currently no registered ${specialtyInfo.nameEn.toLowerCase()} doctors in ${locationInfo.nameEn}.`,
    backToSpecialty: lang === "ne"
      ? `सबै ${specName} हेर्नुहोस्`
      : `Browse All ${specialtyInfo.nameEn} Doctors`,
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <nav className="mb-6 text-sm">
          <ol className="flex items-center gap-2 text-foreground/60">
            ...
            <li>
              <Link
                href={`/${lang}/doctors/specialty/${specialtySlug}`}
            ...
            <li>/</li>
            <li className="text-foreground font-medium">{locName}</li>
          </ol>
        </nav>
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {LOCATIONS.filter((loc) => loc.slug !== locationSlug).map((loc) => (
              <Link
                key={loc.slug}
        ...
        {professionals.length === 0 ? (
          <Card className="p-12 text-center">
            ...
            <Link href={`/${lang}/doctors/specialty/${specialtySlug}`}>
              <Button variant="primary">{t.backToSpecialty}</Button>
            </Link>
          </Card>
        ) : (
          <>
            ...
            {totalPages > 1 && (
              <nav className="mt-12 flex justify-center">
                <div className="flex items-center gap-2">
                  {currentPage > 1 ? (
                    <Link href={buildPageUrl(lang, specialtySlug, locationSlug, currentPage - 1)}>
                  ...
                  <div className="flex items-center gap-1">
                    {generatePageNumbers(currentPage, totalPages).map((pageNum, index) => {
                      ...
                      return (
                        <Link
                          key={pageNum}
                          href={buildPageUrl(lang, specialtySlug, locationSlug, pageNum)}
                        >
                          <Button
